...
Node* buildMooreMachine(Node** nodes) {
    ...
    for (int i = 0; i < node_size; i++) {
        for (int j = 1; j < (alphabet_size * 2) + 1; j+=2) {
            nodes[i]->transactions[k] = malloc(sizeof(Transaction));
            nodes[i]->transactions[k]->next = nodes[matrix[i][j]];
            nodes[i]->transactions[k]->character = alphabet[k];
            nodes[i]->transactions[k]->output = matrix[i][j + 1];
            k++;
        }
        k = 0;
    }
    ...
    for(int j = 0; j < strlen(word); j++) {
        for(int i = 0; i < alphabet_size; i++) {
            if (ptr->transactions[i]->character == word[j]) {
                char temp = ptr->transactions[i]->output;
                *output_ptr++ = ptr->transactions[i]->output + '0';
...
void readFromTable() {
    ...
    FILE* file = fopen("GECISDIYAGRAMI.txt", "r");
